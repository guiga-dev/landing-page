...
const GridLine = ({ direction, position }) => (
  <div
    className={`absolute ${
      direction === 'horizontal'
        ? `left-0 right-0 h-px top-[${position}%]`
        : `top-0 bottom-0 w-px left-[${position}%]`
...
export const HeroSection = () => {
  ...
  return (
    <section
      id="hero"
      ref={heroRef}
      className="relative min-h-screen flex items-center justify-center overflow-hidden bg-[#030B1A]"
    >
      {/* Grid Background */}
      <div className="absolute inset-0 opacity-30">
        {[10, 20, 30, 40, 50, 60, 70, 80, 90].map((pos) => (
          <div key={`h-${pos}`} className="absolute left-0 right-0 h-px bg-blue-500/5" style={{ top: `${pos}%` }} />
        ))}
        {[10, 20, 30, 40, 50, 60, 70, 80, 90].map((pos) => (
          <div key={`v-${pos}`} className="absolute top-0 bottom-0 w-px bg-blue-500/5" style={{ left: `${pos}%` }} />
        ))}
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8 text-center">
        ...
        <motion.h1
          ...
          className="text-5xl sm:text-6xl lg:text-8xl font-bold text-white leading-tight tracking-tight"
        >
          Sua marca em
          <br />
        ...
        >
          ...
          <Button
            ...
            className="group text-white/70 hover:text-white hover:bg-white/5 font-medium px-8 py-6 text-lg rounded-full transition-all duration-300"
          >
